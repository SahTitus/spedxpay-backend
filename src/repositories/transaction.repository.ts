...
export class TransactionRepository extends BaseRepository<ITransaction> {
  ...
  async findUserTransactionsWithPagination(params: {
    ...
    const { userId, type, transactionType, status, search, cardType, page, limit, sort = "createdAt:desc" } = params
    ...
    if (!type || type === "all" || type === "crypto") {
      ...
      if (transactionType) {
        ...
        cryptoQuery.type = { $in: [TRANSACTION_TYPE.BUY_CRYPTO, TRANSACTION_TYPE.SELL_CRYPTO] }
      }
    ...
    if (!type || type === "all" || type === "giftcard") {
      ...
      if (search) {
        ...
        if (giftCardQuery.$or) {
          ...
          giftCardQuery.$and = [{ $or: giftCardQuery.$or }, { $or: searchConditions }]
      ...
      giftCards.forEach((gc: any) => {
        allResults.push({
          ...
          type: gc.sellerId ? TRANSACTION_TYPE.SELL_GIFT_CARD : TRANSACTION_TYPE.BUY_GIFT_CARD,
          status: gc.status,
  ...
  async getUserTransactionStats(userId: string) {
    ...
    // Combine all stats
    const byType = [...cryptoStats, ...giftCardSellStats, ...giftCardBuyStats, ...googleVoiceStats]
    ...
    const giftCardStatusStats = await GiftCard.aggregate([
      { $match: { $or: [{ sellerId: userId as any }, { buyerId: userId as any }] } },
      {
        $group: {
          _id: "$status",
          count: { $sum: 1 },
    ...
    ;[...cryptoStatusStats, ...giftCardStatusStats, ...googleVoiceStatusStats].forEach((stat) => {
