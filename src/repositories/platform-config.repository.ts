...
export class PlatformConfigRepository extends BaseRepository<IPlatformConfig> {
  ...
  async getWalletAddresses() {
    const configs = await this.model
      .find({
        key: { $in: ["wallet_btc", "wallet_eth", "wallet_usdt", "wallet_ltc", "wallet_xrp"] },
      })
      .exec()
  ...
  async getPaymentDetails() {
    const configs = await this.model
      .find({
        key: { $in: ["momo_number", "momo_provider", "bank_name", "bank_account_number", "bank_account_name"] },
      })
      .exec()
    ...
    return {
      ...
      bank: {
        ...
        accountNumber: configs.find((c) => c.key === "bank_account_number")?.value || "",
        accountName: configs.find((c) => c.key === "bank_account_name")?.value || "",
      },
    }
  }
}
