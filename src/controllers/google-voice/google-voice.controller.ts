...
export class GoogleVoiceController {
  ...
  createOrder = asyncHandler(
    async (req: AuthRequest, res: Response, next: NextFunction) => {
      ...
      const result = await this.googleVoiceService.createOrder(
        userId,
        req.body
      );
      res
        .status(201)
        .json(successResponse("Google Voice order created", result));
    }
  );
  ...
  iHavePaid = asyncHandler(
    async (req: AuthRequest, res: Response, next: NextFunction) => {
      ...
      const result = await this.googleVoiceService.iHavePaid(
        userId,
        orderId,
        proofOfPayment
      );
  ...
  reportIssue = asyncHandler(
    async (req: AuthRequest, res: Response, next: NextFunction) => {
      const userId = req.user!.userId;
      const { orderId } = req.params;
      const result = await this.googleVoiceService.reportIssue(
        userId,
        orderId,
        req.body
      );
      res
        .status(200)
        .json(successResponse("Issue reported successfully", result));
    }
  );
  ...
  getUserOrders = asyncHandler(
    async (req: AuthRequest, res: Response, next: NextFunction) => {
      ...
      const orders = await this.googleVoiceService.getUserOrders(
        userId,
        status as string
      );
  ...
  checkDisputeWindow = asyncHandler(
    async (req: AuthRequest, res: Response, next: NextFunction) => {
      const { orderId } = req.params;
