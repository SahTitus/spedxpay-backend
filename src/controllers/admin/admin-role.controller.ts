...
export class AdminRoleController {
  ...
  assignRole = asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const user = await this.adminRoleService.assignRole(superAdminId, userId, role)
  ...
  inviteAdmin = asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const invitation = await this.adminRoleService.inviteAdmin(superAdminId, email, role)

    res.status(201).json(successResponse("Admin invitation sent successfully", invitation))
  })
  ...
  getPendingInvitations = asyncHandler(async (req: AuthRequest, res: Response) => {
    const invitations = await this.adminRoleService.getPendingInvitations()

    res.status(200).json(successResponse("Pending invitations retrieved successfully", invitations))
  })

  cancelInvitation = asyncHandler(async (req: AuthRequest, res: Response) => {
    const { invitationId } = req.params

    const invitation = await this.adminRoleService.cancelInvitation(invitationId)

    res.status(200).json(successResponse("Invitation cancelled successfully", invitation))
  })

  resendInvitation = asyncHandler(async (req: AuthRequest, res: Response) => {
    const { invitationId } = req.params

    const invitation = await this.adminRoleService.resendInvitation(invitationId)

    res.status(200).json(successResponse("Invitation resent successfully", invitation))
  })
}
