...
export class AdminRoleService {
  ...
  async assignRole(superAdminId: string, userId: string, newRole: UserRole) {
    ...
    if (newRole === USER_ROLE.SUPER_ADMIN) {
      throw createError("Cannot assign super admin role", 403, ERROR_CODES.FORBIDDEN)
    }
    ...
    if (user.role === USER_ROLE.SUPER_ADMIN) {
      throw createError("Cannot change super admin role", 403, ERROR_CODES.FORBIDDEN)
    }
  ...
  async inviteAdmin(superAdminId: string, email: string, role: UserRole) {
    // Validate role
    const allowedRoles = [USER_ROLE.ADMIN, USER_ROLE.ASSISTANT_ADMIN] as UserRole[]
    if (!allowedRoles.includes(role)) {
      throw createError("Can only invite admin or assistant admin", 400, ERROR_CODES.INVALID_INPUT)
    }
    ...
    if (existingInvitation && existingInvitation.status === "pending") {
      throw createError("Invitation already sent to this email", 400, ERROR_CODES.INVITATION_ALREADY_EXISTS)
    }
  ...
  async cancelInvitation(invitationId: string) {
    ...
    if (!invitation) {
      throw createError("Invitation not found", 404, ERROR_CODES.INVITATION_NOT_FOUND)
    }

    if (invitation.status !== "pending") {
      throw createError("Can only cancel pending invitations", 400, ERROR_CODES.INVALID_INPUT)
    }

    return this.invitationRepository.update(invitationId, { status: "expired" } as any)
  }
  ...
  async resendInvitation(invitationId: string) {
    ...
    if (invitation.status !== "pending") {
      throw createError("Can only resend pending invitations", 400, ERROR_CODES.INVALID_INPUT)
    }
    ...
    await this.invitationRepository.update(invitationId, { token, expiresAt } as any)

    // Get inviter details
    const inviter = await this.userRepository.findById(invitation.invitedBy.toString())
