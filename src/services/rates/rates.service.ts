...
export class RatesService {
  ...
  async updateRates() {
    try {
      ...
      for (const rate of rates) {
        await this.rateRepo.upsertRate(rate.cryptocurrency, rate.buyRate, rate.sellRate, rate.source, rate.metadata)

        logger.info(`Rate updated: ${rate.cryptocurrency} - Buy: ${rate.buyRate}, Sell: ${rate.sellRate}`)
      }
  ...
  async getRate(cryptocurrency: string) {
    ...
    if (!rate) {
      ...
      logger.warn(`No rate found in DB for ${cryptocurrency}, fetching from provider`)
