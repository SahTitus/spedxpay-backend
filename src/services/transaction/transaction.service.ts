...
export class TransactionService {
  ...
  async getUserTransactions(userId: string, query: GetTransactionsQuery) {
    try {
      ...
      const result =
        await this.transactionRepo.findUserTransactionsWithPagination({
          userId,
  ...
  async cancelTransaction(userId: string, transactionId: string) {
    try {
      ...
      if (transaction) {
        ...
        if (giftCard) {
          ...
          const googleVoiceOrder =
            await this.googleVoiceRepo.findById(transactionId);
      ...
      if (
        ...
        transaction.status !== TRANSACTION_STATUS.UNDER_REVIEW
      ) {
        throw createError(
      ...
      if (model === "transaction") {
        ...
        await this.giftCardRepo.update(transactionId, {
          status: TRANSACTION_STATUS.CANCELLED,
        } as any);
      } else if (model === "googlevoice") {
        await this.googleVoiceRepo.update(transactionId, {
          status: TRANSACTION_STATUS.CANCELLED,
        } as any);
      }
